...
#error unsupported CPU
    ...
    for(i = 0, rel = relocs;i < nb_relocs; i++, rel++) {
	    ...
	    offset < start_offset + (p_end - p_start)) {
            ...
            if(!sym_name)
                continue;
            if (strstart(sym_name, "__op_param", &p) ||
                (strstart(sym_name, "__op_gen_label", &p) && ((uses_goto = 1) == 1))) {
                n = strtoul(p, NULL, 10);
                if (n > MAX_ARGS)
    ...
    if (gen_switch == 2) {
        ...
        if (nb_args > 0) {
            fprintf(outfile, "    extern int ");
            for(i = 0; i < nb_args; i++) {
                if (i != 0)
                    fprintf(outfile, ", ");
                fprintf(outfile, "__op_param%d", i + 1);
            }
            fprintf(outfile, ";\n");
        }
	...
	if (uses_goto)
	  ...
	  fprintf(outfile, "    extern void %s();\n", name);
        ...
        for(i = 0; i < nb_args; i++) {
            fprintf(outfile, "    __op_param%d = *opparam_ptr++;\n", i + 1);
        }
    ...
    for(i = 0; i < nb_args; i++) {
      ...
      fprintf(outfile, "{ const uint16_t *target; int i;\n");
      fprintf(outfile, "  target = opc_buf + gen_labels[__op_param%d];\n", i + 1);
      fprintf(outfile, "extern uint8_t op_nb_args[];\n");
      fprintf(outfile, "while (opc_ptr != target) {\n");
      fprintf(outfile, "  for (i = 0; i < op_nb_args[*opc_ptr]; i++) {\n");
      fprintf(outfile, "      opparam_ptr++;\n");
      fprintf(outfile, "  }\n");
      fprintf(outfile, "  opc_ptr++;\n");
      fprintf(outfile, "}}\n");
...
fprintf(outfile,
"extern \"C\" {\n"
"int interpret(const uint16_t *opc_buf, const uint32_t *opparam_buf, const long *gen_labels)\n"
"{\n"
...
#endif
    fprintf(outfile, "return 0xDEADBEEF;\n");
    fprintf(outfile, "}}\n\n");
